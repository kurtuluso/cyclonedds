        ...
        const DDS_Security_GUID_t *adjusted_participant_guid)
{
    ...
    identity->kagreeAlgoKind = AUTHENTICATION_ALGO_KIND_EC_PRIME256V1;
        ...
        const DDS_Security_IdentityToken *remote_identity_token)
{
    ...
    identity->dsignAlgoKind = AUTHENTICATION_ALGO_KIND_UNKNOWN;
    identity->kagreeAlgoKind = AUTHENTICATION_ALGO_KIND_UNKNOWN;
...
get_dsign_algo_from_string(
    const char *name)
{
    AuthenticationAlgoKind_t algoKind = AUTHENTICATION_ALGO_KIND_UNKNOWN;

    if (name) {
        if (strcmp(AUTH_DSIG_ALGO_RSA_2048_SHA256_IDENT, name) == 0) {
            algoKind = AUTHENTICATION_ALGO_KIND_RSA_2048;
        } else if (strcmp(AUTH_DSIG_ALGO_ECDSA_SHA256_IDENT, name) == 0) {
            algoKind = AUTHENTICATION_ALGO_KIND_EC_PRIME256V1;
        }
    }
        ...
        DDS_Security_SecurityException *ex)
{
    ...
    sz = get_subject_name_DER_encoded(cert, &subject, ex);
    if (sz > 0) {
        ...
        DDS_Security_SecurityException *ex)
{
    ...
    if (!instance || !local_identity_handle || !adjusted_participant_guid || !participant_qos || !candidate_participant_guid) {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "validate_local_identity: Invalid parameter provided");
        goto err_bad_param;
    }
    ...
    if (!identityCertPEM) {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "validate_local_identity: missing property '%s'", PROPERTY_IDENTITY_CERT);
    ...
    if (!identityCaPEM) {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "validate_local_identity: missing property '%s'", PROPERTY_IDENTITY_CA);
    ...
    if (!privateKeyPEM) {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "validate_local_identity: missing property '%s'", PROPERTY_PRIVATE_KEY);
    ...
    if( implementation->trustedCAList._length != 0 ){
        ...
        for (i = 0; i < implementation->trustedCAList._length; ++i) {
            X509_digest(implementation->trustedCAList._buffer[i], digest, hash_buffer_trusted, &size);
        ...
        if (result != DDS_SECURITY_VALIDATION_OK) { /*not trusted*/
            DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_CA_NOT_TRUSTED_CODE, result, DDS_SECURITY_ERR_CA_NOT_TRUSTED_MESSAGE);
        ...
        if (strncmp(AUTH_PROTOCOL_CLASS, token->class_id, sz) == 0) {
            ...
            if (sscanf(ptr, ":%u.%u%1s", &major, &minor, postfix) == 2) {
                if (major == AUTH_PROTOCOL_VERSION_MAJOR) {
                    ...
                    result = DDS_SECURITY_VALIDATION_FAILED;
                    DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "remote identity token: version %u.%u not supported", major, minor);
                }
            } else {
                result = DDS_SECURITY_VALIDATION_FAILED;
                DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE,  result, "remote identity token: class_id has wrong format");
            }
        } else {
            result = DDS_SECURITY_VALIDATION_FAILED;
            DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "remote identity token: class_id='%s' not supported", token->class_id);
        }
    } else {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result, "remote identity token: class_id is empty");
    }
        ...
        DDS_Security_SecurityException *ex)
{
    ...
    result = dh_get_public_key_value(handshake->ldh, localIdent->kagreeAlgoKind, &dhPubKeyData, &dhPubKeyDataSize, ex);
    if (result != DDS_SECURITY_VALIDATION_OK) {
    ...
    DDS_Security_SecurityException *ex)
{
    ...
    if( trusted_ca_list->_length == 0 ){ //no trusted set. check with local CA
        ...
        for (i = 0; i < trusted_ca_list->_length; ++i) {
            /* We'll only return the exception of the last one, if it failed. */
            DDS_Security_Exception_reset(ex);
            result = verify_certificate(identityCert, trusted_ca_list->_buffer[i], ex);
            if (result == DDS_SECURITY_VALIDATION_OK) {
                break;
            }
        }
    ...
    if (dsignAlgoKind == AUTHENTICATION_ALGO_KIND_UNKNOWN) {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result,
    ...
    if (kagreeAlgoKind == AUTHENTICATION_ALGO_KIND_UNKNOWN) {
        result = DDS_SECURITY_VALIDATION_FAILED;
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, result,
    ...
    result = dh_read_public_key_by_value(&pdhkey, kagreeAlgoKind, dh1->value._buffer, dh1->value._length, ex);
    if (result != DDS_SECURITY_VALIDATION_OK) {
        goto err_no_dh;
    }
    ...
    DDS_Security_SecurityException *ex)
{
    ...
    *pdhkey = NULL;
    result = dh_read_public_key_by_value(pdhkey, kagreeAlgoKind, dh2->value._buffer, dh2->value._length, ex);
    if (result != DDS_SECURITY_VALIDATION_OK) {
        goto err_inv_dh;
    }
        ...
        DDS_Security_SecurityException *ex)
{
    ...
    result = dh_get_public_key_value(handshake->ldh, remoteIdent->kagreeAlgoKind, &dhPubKeyData, &dhPubKeyDataSize, ex);
    if (result != DDS_SECURITY_VALIDATION_OK) {
        goto err_get_public_key;
    }
    ...
    DDS_Security_SecurityException *ex)
{
    ...
    *shared_secret = NULL;
    ...
    if (!ctx)
    {
        ...
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, DDS_SECURITY_VALIDATION_FAILED,
                        "process_handshake: Shared secret failed to create context: %s", msg);
        ddsrt_free(msg);
        goto fail_ctx_new;
    }
    ...
    if (EVP_PKEY_derive_init(ctx) <= 0)
    {
        char *msg = get_openssl_error_message();
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, DDS_SECURITY_VALIDATION_FAILED,
                        "process_handshake: Shared secret failed to initialize context: %s", msg);
        ddsrt_free(msg);
        goto fail_derive;
    }
    if (EVP_PKEY_derive_set_peer(ctx, handshake->rdh) <= 0)
    {
        char *msg = get_openssl_error_message();
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, DDS_SECURITY_VALIDATION_FAILED,
                        "process_handshake: Shared secret failed to set peer key: %s", msg);
        ddsrt_free(msg);
        goto fail_derive;
    }
    ...
    if (EVP_PKEY_derive(ctx, NULL, &skeylen) <= 0)
    {
        char *msg = get_openssl_error_message();
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, DDS_SECURITY_VALIDATION_FAILED,
                        "process_handshake:  Shared secret failed to determine key length: %s", msg);
        ddsrt_free(msg);
        goto fail_derive;
    }
    ...
    if (EVP_PKEY_derive(ctx, secret, &skeylen) <= 0)
    {
        char *msg = get_openssl_error_message();
        DDS_Security_Exception_set(ex, DDS_AUTH_PLUGIN_CONTEXT, DDS_SECURITY_ERR_UNDEFINED_CODE, DDS_SECURITY_VALIDATION_FAILED,
                "process_handshake: Could not compute the shared secret: %s", msg);
        ddsrt_free(msg);
        goto fail_derive;
    }
...
int init_authentication( const char *argument, void **context)
{
...
int finalize_authentication(void *instance)
{
    ...
    if( authentication ){
        ...
        clean_ca_list(&(authentication->trustedCAList));
