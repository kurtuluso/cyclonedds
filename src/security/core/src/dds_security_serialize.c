...
#define bswap2(x) ((short) bswap2u_ ((unsigned short) (x)))
...
#define toBE2u(x) bswap2u_ (x)
#define toBE4(x) bswap4 (x)
#define toBE4u(x) bswap4u_ (x)
#define toBE8(x) bswap8 (x)
#define toBE8u(x) bswap8u_ (x)
#define fromBE2(x) bswap2 (x)
#define fromBE2u(x) bswap2u_ (x)
#define fromBE4(x) bswap4 (x)
#define fromBE4u(x) bswap4u_ (x)
#define fromBE8(x) bswap8 (x)
#define fromBE8u(x) bswap8u_ (x)
#else
#define toBE2u(x) (x)
#define toBE4(x) (x)
#define toBE4u(x) (x)
...
DDS_Security_Serialize_uint8(
     ...
     unsigned char value)
{
    serbuffer_adjust_size(serializer, sizeof(unsigned char));
     ...
     unsigned short value)
{
    ...
    *(unsigned short *)&(ser->buffer[ser->offset]) = toBE2u(value);
     ...
     uint32_t value)
{
    ...
    *(uint32_t *)&(ser->buffer[ser->offset]) = toBE4u(value);
     ...
     const DDS_Security_DataHolderSeq *seq)
{
    ...
    DDS_Security_Serialize_uint32_t(ser, seq->_length);
    for (i = 0; i < seq->_length; i++) {
        DDS_Security_Serialize_DataHolder(ser, &seq->_buffer[i]);
    }
}
     ...
     unsigned short *value)
{
    ...
    *value = fromBE2u(*(unsigned short *)dser->cursor);
    dser->cursor += l;
    dser->remain -= l;
     ...
     uint32_t *value)
{
    ...
    *value = fromBE4u(*(uint32_t *)dser->cursor);
    dser->cursor += l;
    dser->remain -= l;
